...
COPY server.py admin.py local_embeddings.py gunicorn.conf.py .
COPY public/ ./public/
RUN useradd --uid 10001 --create-home ciudad && mkdir -p /data && chown ciudad:ciudad /data
USER ciudad
ENV DATA_DIR=/data PORT=8080 PYTHONUNBUFFERED=1 APP_ENV=production
EXPOSE 8080
...
CMD ["gunicorn", "--config", "gunicorn.conf.py", "server:create_app(initialize=False)"]
