const CACHE = 'ciudad-visible-shell-v3.0';
...
self.addEventListener('activate', e => e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('ciudad-visible-shell-') && k !== CACHE).map(k => caches.delete(k))))));
